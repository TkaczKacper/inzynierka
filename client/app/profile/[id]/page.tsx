   ...
   useEffect(() => {
      const userStats = async () => {
         const res = await axios.get(
            `${backend_url}/strava/get-athlete-stats`,
            {
         ...
         {athleteStats ? (
            <div>
               <div>
                  ...
                  <div>
                     Avg Distance / Week:{" "}
                     {(athleteStats.recentRideTotals.distance / 4000).toFixed(
                        2
                     )}
                     km
                  </div>
                  <div>
                     Elev Gain / Week:{" "}
                     {(athleteStats.recentRideTotals.elevationGain / 4).toFixed(
                        1
                     )}
                     m
                  </div>
                  <div>
                     Avg Time / Week:{" "}
                     {parseDuration(
                        athleteStats.recentRideTotals.elapsedTime / 4
                     )}
                  </div>
               </div>
               <div>
                  ...
                  <div>
                     Elev Gain: {athleteStats.ytdRideTotals.elevationGain}
                  </div>
                  <div>
                     Time:{" "}
                     {parseDuration(athleteStats.ytdRideTotals.elapsedTime)}
                  </div>
               </div>
               <div>
                  <h3>All-Time</h3>
                  ...
                  <div>
                     Elev Gain: {athleteStats.allTimeRideTotals.elevationGain}
                  </div>
                  <div>
                     Time:{" "}
                     {parseDuration(athleteStats.allTimeRideTotals.elapsedTime)}
                  </div>
